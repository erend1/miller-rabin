...
# Use Euclidean algorithms to find GCD of two integers.
def gcd(a: int, b: int):
    # Begin iteration on the formula.
    while b:
        a, b = b, a % b
    # Return the absolute value of the latest version of a.
    return abs(a)
...
def power_mod(a: int, d: int, n: int):
    result = 1
    while d > 0:
        if d % 2 == 1:
            result = (result * a) % n
